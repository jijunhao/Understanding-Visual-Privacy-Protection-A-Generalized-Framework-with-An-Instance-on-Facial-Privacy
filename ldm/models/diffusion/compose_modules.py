...
class ComposeUNet(nn.Module):
    ...
    def forward(self, x, t, cond):
        ...
        if ('seg_mask' in self.conditions) and ('text' in self.conditions) and ('sketch' not in self.conditions):
            ...
            if self.return_confidence_map:
                if self.return_each_branch:
                    return {'outputs': output, 'seg_mask_confidence_map': seg_mask_confidence_map, 'text_confidence_map': text_confidence_map, 'sketch_confidence_map': sketch_confidence_map,'seg_mask_unet_output': seg_mask_unet_output, 'text_unet_output': text_unet_output, 'sketch_unet_output': sketch_unet_output }
                else:
                    return {'outputs': output, 'seg_mask_confidence_map': seg_mask_confidence_map, 'text_confidence_map': text_confidence_map, 'sketch_confidence_map': sketch_confidence_map}
            elif self.return_each_branch:
                    return {'outputs': output, 'text_unet_output': text_unet_output, 'seg_mask_unet_output': seg_mask_unet_output, 'sketch_unet_output': sketch_unet_output}
            else:
                return output
