...
def disabled_train(self, mode=True):
    """Overwrite models.train with this function to make sure train/eval mode
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                ...
                print(f"Training {self.__class__.__name__} as an unconditional models.")
                self.cond_stage_model = None
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                # 2. apply models loop over last dim
    ...
    def _rescale_annotations(self, bboxes, crop_coordinates):  # TODO: move to datasets
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation",
                ...
                assert isinstance(cond, dict), 'cond must be dict to be fed into models'
            ...
            # apply models by loop over crops
            output_list = [self.model(z_list[i], t, **cond_list[i]) for i in range(z.shape[-1])]
            assert not isinstance(output_list[0],
                                  tuple)  # todo cant deal with multiple models outputs check this never happens
    ...
    def configure_optimizers(self):
        ...
        if self.learn_logvar:
            print('Diffusion models optimizing logvar')
...
"""
修改了get_input(),输入是一个字典类型数据，其中包含多个条件数据
"""