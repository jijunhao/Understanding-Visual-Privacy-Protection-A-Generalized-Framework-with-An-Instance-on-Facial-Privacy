...
class BERTEmbedder(AbstractEncoder):
    """Uses the BERT tokenizr models and add some transformer encoder layers"""
...
class IdAndTextEncoder(nn.Module):
    ...
    def forward(self, input):
        ...
        id = TestFace.pred_id(input['id'],'irse50',TestFace.targe_models)
        id = id.unsqueeze(1)

        #print(id.shape)
        if self.use_tknz_fn:
            tokens = self.tknz_fn(input['text'])#.to(self.device)
        else:
            tokens = input['text']
        text = self.transformer(tokens, return_embeddings=True)
        #print(text.shape)
        id_text = torch.cat([id, text], 1)
...
class IdEncoder(nn.Module):
    ...
    def forward(self, input):
        # image 输入
        # TestFace = indentity.TestFace()
        # id = TestFace.pred_id(input,'ir152',TestFace.targe_models)
        id = input
...
"""
stable-diffusion-pytorch版本，增加了PassSegMaskEncoder、SegMaskAndTextEncoder、SegMaskEncoder、IdAndTextEncoder、IdEncoder
"""