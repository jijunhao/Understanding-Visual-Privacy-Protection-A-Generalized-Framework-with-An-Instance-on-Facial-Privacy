...
class WeightStandardizedConv2d(nn.Conv2d):
    ...
    对卷积核进行权重标准化和组归一化两种方法在一起使用可以产生协同效应，提高模型的性能、
    """
    def forward(self, x):
...
# 位置编码组件 为了让网络知道当前处理的是去噪过程中的哪一个step
...
class Block(nn.Module):
    ...
    输入(B,dim,H,W)
    输出((B,dim_out,H,W)
    conv-->norm(-->利用time_emb进行图像调整)-->lu
    """
    def __init__(self, dim, dim_out, groups = 8):
        super().__init__()
        self.proj = WeightStandardizedConv2d(dim, dim_out, 3, padding = 1)
        # self.proj = nn.Conv2d(dim, dim_out, 3, padding=1) 更简单的实现 when group=1
...
class ResnetBlock(nn.Module):
    """
    输入(B,dim,H,W)和time_emb
    输出((B,dim_out,H,W)

    time_emb-->MLP-->
                    |
    feature map-->Block-->Block-->resnet
    |                              |
    ------------->conv------------->

    最下面的conv是为了保持dim_out一致
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 传入的x为（B,C,H,W）
        # 经过查询变成batch,(head,channel),x,y
        # 对每个通道图像展开成一个向量 batch,head,channel,(x,y)
...
class Unet(nn.Module):
    ...
    condition->
              |
    image-------->7*7 conv-->[down sample]-->Resnet block-->attention-->Resnet block-->[up sample]-->conv

    down sample: Resnet block-->Resnet block-->attention-->down sample
                        |           |
    up sample:   Resnet block-->Resnet block-->attention-->up sample
                        |           |
                     time emb    time emb
    """
    def __init__(
